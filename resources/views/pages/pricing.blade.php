...
@section('content')
    ...
    <section style="background:linear-gradient(160deg,#0A1520 0%,#0D1B2A 55%,#101F2E 100%);padding:88px 0 72px;border-bottom:2px solid #AE8225;position:relative;overflow:hidden;">
        ...
        <div class="container text-center" style="position:relative;z-index:2;">
            ...
            <div style="display:inline-block;font-family:'Cinzel',serif;font-size:9px;color:#D4A843;letter-spacing:5px;text-transform:uppercase;padding:7px 22px;border:1px solid rgba(174,130,37,.4);margin-bottom:22px;">✦ {{ section('pricing', 'hero_eyebrow', 'title', 'Transparent Pricing') }} ✦</div>
            ...
            <!-- 4 trust badges -->
            @php
                $pricingBadgesRaw = section('pricing', 'trust_badges', 'title', '3-Day Free Trial,One-on-One Classes,Female Teachers Available,Cancel Anytime');
                $pricingBadges = array_map('trim', explode(',', $pricingBadgesRaw));
            @endphp
            <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:center;">
                @foreach($pricingBadges as $badge)
    ...
    <section style="background:#f5f3ed;padding:80px 0 70px;">
        <div class="container">
            ...
            <p class="text-center mt-4" style="font-family:'Lato',sans-serif;font-size:13px;color:#aaa;">
                ...
                &nbsp;{{ section('pricing', 'bottom_note', 'title', 'Fees are discussed and confirmed via WhatsApp after your free trial. No upfront payment.') }}
            </p>
        </div>
    </section>
    ...
    <section style="background:#0D1B2A;padding:72px 0;border-top:2px solid #AE8225;border-bottom:2px solid #AE8225;">
        <div class="container">
            <div class="text-center mb-5">
                <span style="font-family:'Cinzel',serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;color:#AE8225;display:block;margin-bottom:12px;">{{ section('pricing', 'compare_title', 'description', 'Side by Side') }}</span>
                <h2 style="font-family:'Cinzel',serif;color:#fff;font-size:clamp(20px,3vw,34px);font-weight:800;">{{ section('pricing', 'compare_title', 'title', 'Full Plan Comparison') }}</h2>
                ...
                <table class="bia-table">
                    <thead>
                        <tr>
                            ...
                            <th class="bia-th-b">Plan B ✦<br><small>Most Popular</small></th>
                            <th>Plan C<br><small>Premium</small></th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            $compareRaw = section('pricing', 'compare_rows', 'title', 'Days per Week|3 days|5 days|6 days,Class Duration|30 min|30 min|30 min,Classes per Month|12|20|24,Courses|1|2|All,Progress Report|Monthly|Weekly|Weekly,Parent-Teacher Call||Monthly|Weekly,Certificate||✓|✓,Makeup Classes|✓|✓|Priority,Female Teacher|✓|✓|✓,WhatsApp Support|✓|✓|Dedicated,Free Trial|✓|✓|✓');
                            $rows = [];
                            foreach(explode(',', $compareRaw) as $row) {
                                $cols = array_map('trim', explode('|', $row));
                                if(count($cols) >= 4) {
                                    // convert string booleans
                                    for($c = 1; $c <= 3; $c++) {
                                        if($cols[$c] === '✓') $cols[$c] = true;
                                        elseif($cols[$c] === '' || $cols[$c] === 'false') $cols[$c] = false;
                                    }
                                    $rows[] = $cols;
                                }
                            }
    ...
    <section style="background:#fff;padding:72px 0;">
        <div class="container">
            <div class="text-center mb-5">
                <span style="font-family:'Cinzel',serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;color:#AE8225;display:block;margin-bottom:12px;">{{ section('pricing', 'courses_title', 'description', 'What We Teach') }}</span>
                <h2 style="font-family:'Cinzel',serif;color:#0D1B2A;font-size:clamp(20px,3vw,34px);font-weight:800;">{{ section('pricing', 'courses_title', 'title', 'All Available Courses') }}</h2>
                <div style="width:40px;height:2px;background:linear-gradient(90deg,#AE8225,#D4A843);margin:14px auto 0;"></div>
            </div>
            <div class="row g-3">
                @php
                    $coursesRaw = section('pricing', 'courses_list', 'title', 'fa-book|Norani Qaida|For complete beginners & young children,fa-quran|Quran Recitation (Nazra)|Learn to read the Holy Quran correctly,fa-microphone|Tajweed ul Quran|Rules of beautiful Quranic recitation,fa-star|Hifz ul Quran|Complete Quran memorisation programme,fa-mosque|Islamic Studies|Aqeedah, Fiqh, Seerah & Islamic manners,fa-language|Arabic Language|Classical Arabic to understand the Quran,fa-scroll|Hadith & Seerah|40 Hadith Nawawi & Prophet\'s ﷺ life,fa-hands|Namaz & Ibadat|Correct method of Salah & worship');
                    $courses = [];
                    foreach(explode(',', $coursesRaw) as $c) {
                        $parts = array_map('trim', explode('|', $c));
                        if(count($parts) >= 3) $courses[] = $parts;
                    }
                @endphp
    ...
    <section style="background:#0D1B2A;padding:72px 0;border-top:2px solid #AE8225;position:relative;overflow:hidden;">
        ...
        <div class="container text-center" style="position:relative;z-index:2;">
            ...
            <h2 style="font-family:'Cinzel',serif;color:#fff;font-size:clamp(22px,3.5vw,40px);font-weight:900;margin-bottom:12px;">
                {!! section('pricing', 'cta', 'title', 'Try 3 Classes <span style="color:#D4A843;">Completely Free</span>') !!}
            </h2>
            <p style="font-family:'Lato',sans-serif;color:rgba(255,255,255,.5);font-size:15px;max-width:440px;margin:0 auto 30px;line-height:1.85;">{{ section('pricing', 'cta', 'description', 'No payment. No commitment. Just 3 live classes with a certified teacher.') }}</p>
            <div style="display:flex;flex-wrap:wrap;gap:12px;justify-content:center;">
                <a href="{{ section('pricing', 'cta', 'button_url', '/free-trial') }}" style="display:inline-flex;align-items:center;gap:10px;background:#AE8225;color:#fff;font-family:'Cinzel',serif;font-weight:700;font-size:11px;letter-spacing:2px;text-transform:uppercase;padding:15px 36px;border:2px solid #AE8225;text-decoration:none;transition:all .3s;" onmouseover="this.style.background='transparent';this.style.color='#AE8225';" onmouseout="this.style.background='#AE8225';this.style.color='#fff';">
                    <i class="fas fa-play-circle"></i> {{ section('pricing', 'cta', 'button_text', 'Book Free Trial') }}
                </a>
