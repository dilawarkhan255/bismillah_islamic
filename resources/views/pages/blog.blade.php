...
@section('content')
    ...
    <section class="bbl-featured">
                ...
                <div class="col-lg-7">
                    <nav style="margin-bottom:22px;">
                        <ol class="breadcrumb mb-0" style="background:transparent; padding:0;">
                            ...
                            <li style="color:var(--gold-light); font-family:'Lato',sans-serif; font-size:13px; list-style:none;">Blog</li>
                        </ol>
                    </nav>

                    <span class="bbl-feat-cat" style="background:#2e7d32;">Quran Learning</span>
                    <h1 class="bbl-feat-title bbl-reveal" style="animation-delay:.1s;">10 Proven Tips To Memorize<br class="d-none d-md-inline">The Quran Faster</h1>
                    ...
                    <p class="bbl-feat-excerpt bbl-reveal" style="animation-delay:.4s;">
                        Memorizing the Quran is one of the most noble acts a Muslim can pursue. We share 10 proven techniques used by our expert Hifz teachers.
                    </p>
        ...
        <div class="bbl-particles">
            @for($p = 1; $p <= 8; $p++)
                <div class="bbl-particle" style="left:{{ rand(5, 95) }}%; animation-delay:{{ $p * .6 }}s; animation-duration:{{ 4 + ($p % 3) }}s;"></div>
            @endfor
        </div>
    </section>
                    ...
                    @foreach($posts as $pi => $post)
                        <article class="bbl-card bbl-sr" data-dir="up" style="transition-delay:{{ ($pi % 3) * .1 }}s;">
                            ...
                            <div class="bbl-card-thumb">
                                ...
                                <span class="bbl-card-badge" style="background:{{ $post['cat_color'] }};">{{ $post['cat'] }}</span>
                ...
                <aside class="col-lg-4 bbl-sr" data-dir="right">
                        ...
                        <div class="bbl-widget-bd bbl-widget-bd--flush">
                            @foreach($categories as $cat)
                                <a href="#" class="bbl-cat">
                                    ...
                                    <span class="bbl-cat-count" style="color:{{ $cat['color'] }}; border-color:{{ $cat['color'] }}33;">{{ $cat['count'] }}</span>
                                </a>
                            @endforeach
                        </div>
                    </div>
                            ...
                            @foreach($recent as $ri => $r)
                                <a href="#" class="bbl-recent {{ $ri < count($recent) - 1 ? 'bbl-recent--sep' : '' }}">
                                    ...
                                    <div class="bbl-recent-text">
                                        ...
                                        <span class="bbl-recent-date"><i class="fas fa-calendar-alt"></i> {{ $r['date'] }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                    ...
                    <div class="bbl-enroll-cta">
                        ...
                        <div class="bbl-enroll-inner">
                            ...
                            <p class="bbl-enroll-text">Join Bismillah Islamic Academy and walk the path of Quranic excellence today.</p>
...
@media (max-width: 767px) {
    .bbl-featured { min-height: 420px; padding: 60px 0 50px; }
}
@media (max-width: 575px) {
    .bbl-featured { min-height: 360px; padding: 50px 0 40px; }
}
...
.bbl-feat-title {
    ...
    font-size: clamp(22px, 3.5vw, 46px); /* ✅ responsive font */
    font-weight: 900; color: #fff; line-height: 1.25; margin-bottom: 20px;
}
.bbl-feat-meta {
...
.bbl-feat-meta i { color: var(--gold); }
.bbl-feat-excerpt {
...
.bbl-feat-btn:hover { background: transparent; color: var(--gold-light); }
.bbl-feat-btn i { transition: transform .3s; }
.bbl-feat-btn:hover i { transform: translateX(5px); }

/* PARTICLES */
.bbl-particles { position: absolute; inset: 0; pointer-events: none; overflow: hidden; z-index: 2; }
.bbl-particle {
    position: absolute; width: 3px; height: 3px;
    background: var(--gold); border-radius: 50%; opacity: 0;
    bottom: -10px; animation: bblFloat linear infinite;
}
@keyframes bblFloat {
...
/* REVEAL */
.bbl-reveal { opacity: 0; transform: translateY(24px); animation: bblRev .8s cubic-bezier(.25,.46,.45,.94) forwards; }
@keyframes bblRev { to { opacity: 1; transform: translateY(0); } }

/* SCROLL REVEAL */
.bbl-sr { opacity: 0; transition: opacity .75s ease, transform .75s cubic-bezier(.25,.46,.45,.94); }
.bbl-sr[data-dir="up"]    { transform: translateY(40px); }
.bbl-sr[data-dir="right"] { transform: translateX(45px); }
.bbl-sr.in { opacity: 1 !important; transform: translate(0) !important; }

/* ═══════════════════════════════════════
   MAIN SECTION
═══════════════════════════════════════ */
.bbl-main { background: var(--off-white); padding: 80px 0 100px; }

.bbl-posts-heading {
    display: flex; align-items: flex-end;
    justify-content: space-between; margin-bottom: 36px; gap: 20px;
}
.bbl-gold-rule {
    flex: 1; height: 1px;
...
.bbl-h2 {
    ...
    color: var(--navy); font-weight: 800; line-height: 1.1; margin: 0;
}
.bbl-h2 span { color: var(--gold); }

/* ═══════════════════════════════════════
   BLOG CARDS — RESPONSIVE IMAGE FIX
═══════════════════════════════════════ */
.bbl-card {
    display: flex;
...
.bbl-card:hover::before { transform: scaleY(1); }

/* ✅ THUMBNAIL — desktop: fixed width, auto height */
.bbl-card-thumb {
    width: 220px;
    flex-shrink: 0;
    position: relative;
...
/* ✅ MOBILE: stack vertically, full width, fixed reasonable height */
@media (max-width: 767px) {
    .bbl-card { flex-direction: column; }
    .bbl-card-thumb {
...
@media (max-width: 575px) {
    .bbl-card-thumb { height: 180px; }
}

.bbl-card-thumb img {
    width: 100%;
    height: 100%;           /* ✅ fill the thumb container */
    min-height: 0;          /* ✅ remove old min-height:230px — was forcing overflow */
    object-fit: cover;
    display: block;
    transition: transform .55s ease;
}

/* Desktop: thumb stretches to card height naturally */
@media (min-width: 768px) {
    ...
    .bbl-card-thumb { position: relative; }
}

.bbl-card:hover .bbl-card-thumb img { transform: scale(1.08); }
.bbl-card-thumb-overlay {
    position: absolute; inset: 0; background: rgba(13,27,42,0); transition: background .4s;
}
.bbl-card:hover .bbl-card-thumb-overlay { background: rgba(13,27,42,.2); }
...
.bbl-card-read-badge {
    ...
    background: rgba(13,27,42,.8); color: var(--gold-light);
    font-family: 'Lato', sans-serif; font-size: 10px;
    font-weight: 700; letter-spacing: 1px; padding: 4px 10px; z-index: 2;
...
@media (max-width: 575px) {
    .bbl-card-body { padding: 18px 18px 16px; }
}
.bbl-card-meta {
...
.bbl-card-meta i { color: var(--gold); font-size: 11px; }
.bbl-card-title {
    font-family: 'Cinzel', serif; font-size: 16px;
    font-weight: 700; color: var(--navy); line-height: 1.45; margin-bottom: 10px;
}
@media (max-width: 575px) { .bbl-card-title { font-size: 14px; } }
.bbl-card-link { color: var(--navy); text-decoration: none; transition: color .3s; }
.bbl-card:hover .bbl-card-link { color: var(--gold-dark); }
.bbl-card-excerpt {
...
.bbl-card-footer {
    ...
    padding-top: 14px; border-top: 1px solid rgba(174,130,37,.1);
...
.bbl-read-more i { transition: transform .3s; font-size: 13px; }
.bbl-read-more:hover { color: var(--gold-dark); }
.bbl-read-more:hover i { transform: translateX(6px); }
.bbl-card-icons { display: flex; gap: 8px; }
.bbl-icon-btn {
    width: 32px; height: 32px;
    border: 1px solid rgba(174,130,37,.2); background: transparent;
    color: var(--text-muted); font-size: 12px; cursor: pointer;
    display: flex; align-items: center; justify-content: center; transition: all .3s;
}
.bbl-icon-btn:hover { background: var(--gold); color: #fff; border-color: var(--gold); }
.bbl-card-number {
    position: absolute; bottom: -10px; right: 14px;
    font-family: 'Cinzel', serif; font-size: 70px; font-weight: 900;
    line-height: 1; color: rgba(174,130,37,.06); pointer-events: none;
    user-select: none; transition: color .4s;
}
.bbl-card:hover .bbl-card-number { color: rgba(174,130,37,.1); }

/* PAGINATION */
.bbl-pagination { display: flex; align-items: center; gap: 8px; margin-top: 8px; }
.bbl-pg {
    ...
    border: 1px solid rgba(174,130,37,.2); transition: all .3s; padding: 0 12px;
}
.bbl-pg:hover,
.bbl-pg--active { background: var(--gold) !important; color: #fff !important; border-color: var(--gold) !important; }
.bbl-pg-dots { color: var(--text-muted); font-size: 16px; letter-spacing: 2px; line-height: 1; }

/* SIDEBAR WIDGETS */
.bbl-widget {
    background: var(--white); border: 1px solid rgba(174,130,37,.12);
...
.bbl-widget-hd {
    ...
    color: var(--gold-light); display: flex; align-items: center; gap: 9px;
    border-bottom: 1px solid rgba(174,130,37,.2);
}
.bbl-widget-hd i { color: var(--gold); font-size: 12px; }
.bbl-widget-bd { padding: 18px 20px; }
.bbl-widget-bd--flush { padding: 0; }

.bbl-widget--search { margin-bottom: 24px; }
.bbl-search { display: flex; }
.bbl-search input {
    flex: 1; padding: 11px 14px;
    border: 1px solid rgba(174,130,37,.2); border-right: none;
    background: var(--off-white); font-family: 'Lato', sans-serif;
    font-size: 13px; color: var(--text-dark); outline: none; transition: border-color .3s;
}
.bbl-search input:focus { border-color: var(--gold); }
.bbl-search button {
    width: 44px; background: var(--gold); border: none;
    color: #fff; font-size: 13px; cursor: pointer; transition: background .3s; flex-shrink: 0;
}
.bbl-search button:hover { background: var(--gold-dark); }

/* QURAN VERSE */
.bbl-verse {
    position: relative; overflow: hidden; background: var(--navy);
    border: 1px solid rgba(174,130,37,.25); margin-bottom: 24px;
...
.bbl-verse-inner { position: relative; z-index: 2; }
.bbl-verse-arabic {
    font-family: 'Amiri', serif; font-size: 22px;
    color: var(--gold-light); line-height: 1.9; margin-bottom: 12px; direction: rtl;
}
.bbl-verse-line { width: 40px; height: 1px; background: var(--gold); margin: 0 auto 12px; }
.bbl-verse-trans {
    font-family: 'Amiri', serif; font-size: 14px;
    color: rgba(255,255,255,.65); font-style: italic; margin: 0 0 8px; line-height: 1.7;
}
.bbl-verse-ref { font-family: 'Lato', sans-serif; font-size: 10px; color: var(--gold); letter-spacing: 2px; text-transform: uppercase; }

/* CATEGORIES */
.bbl-cat {
    display: flex; align-items: center; gap: 10px;
    padding: 11px 20px; border-bottom: 1px solid rgba(174,130,37,.07);
    text-decoration: none; transition: all .3s;
}
.bbl-cat:last-child { border-bottom: none; }
.bbl-cat:hover { background: rgba(174,130,37,.04); padding-left: 26px; }
.bbl-cat-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; transition: transform .3s; }
.bbl-cat:hover .bbl-cat-dot { transform: scale(1.5); }
.bbl-cat-name { font-family: 'Lato', sans-serif; font-size: 13px; color: var(--text-mid); flex: 1; transition: color .3s; }
.bbl-cat:hover .bbl-cat-name { color: var(--gold-dark); }
.bbl-cat-count { font-family: 'Lato', sans-serif; font-size: 11px; font-weight: 700; padding: 2px 9px; border: 1px solid; }

/* RECENT */
.bbl-recent {
    display: flex; gap: 12px; align-items: center;
    padding: 13px 20px; text-decoration: none; transition: all .3s;
}
.bbl-recent--sep { border-bottom: 1px solid rgba(174,130,37,.08); }
.bbl-recent:hover { background: rgba(174,130,37,.04); }
.bbl-recent-thumb {
    width: 64px; height: 56px; flex-shrink: 0; overflow: hidden; position: relative;
}
.bbl-recent-thumb img {
    width: 100%; height: 100%; object-fit: cover; transition: transform .4s;
}
.bbl-recent:hover .bbl-recent-thumb img { transform: scale(1.1); }
.bbl-recent-overlay { position: absolute; inset: 0; background: rgba(13,27,42,0); transition: background .3s; }
.bbl-recent:hover .bbl-recent-overlay { background: rgba(13,27,42,.15); }
.bbl-recent-title {
    font-family: 'Cinzel', serif; font-size: 12px; color: var(--navy);
    margin: 0 0 5px; line-height: 1.4; transition: color .3s;
}
.bbl-recent:hover .bbl-recent-title { color: var(--gold-dark); }
.bbl-recent-date {
    font-family: 'Lato', sans-serif; font-size: 11px; color: var(--gold);
...
.bbl-enroll-cta {
    position: relative; overflow: hidden; background: var(--navy);
    border: 1px solid rgba(174,130,37,.25); padding: 34px 24px;
}
.bbl-enroll-bg {
...
.bbl-enroll-cta::before {
    ...
    background-size: 200% 100%; animation: bblShimmer 2.5s linear infinite;
}
@keyframes bblShimmer { 0% { background-position: 0% 0%; } 100% { background-position: 200% 0%; } }
.bbl-enroll-inner { position: relative; z-index: 2; text-align: center; }
.bbl-enroll-icon { font-size: 2.6rem; color: var(--gold-light); margin-bottom: 14px; display: block; animation: bblPulse 2.5s ease-in-out infinite; }
@keyframes bblPulse { 0%,100% { opacity: .8; transform: scale(1); } 50% { opacity: 1; transform: scale(1.08); } }
.bbl-enroll-title { font-family: 'Cinzel', serif; color: #fff; font-size: 16px; font-weight: 700; margin-bottom: 10px; letter-spacing: 1px; }
.bbl-enroll-text { font-family: 'Lato', sans-serif; font-size: 13px; color: rgba(255,255,255,.6); line-height: 1.75; margin-bottom: 20px; }
.bbl-enroll-btn {
...
.bbl-enroll-btn:hover { background: transparent; color: var(--gold-light); }
</style>
@endpush
